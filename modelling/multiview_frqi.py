...
class Multiview_FRQI(tf.keras.layers.Layer):
    ...
    def Multiview_FRQI(self, bits, params):
        ...
        for i in range(self.num_qubits - self.image_color_base-self.num_images_qubits):
            circuit.append(cirq.H(bits[i]))
        for n in range(self.config.NUM_IMAGES):

            pre_position_binary = ""
            for k in range(self.num_qubits - self.image_color_base - self.num_images_qubits):
            ...
            for index_bit in range(self.num_images_qubits):
                if cur_index_binary[index_bit] != pre_index_binary[index_bit]:
                    circuit.append(cirq.X(bits[self.num_qubits_row + self.num_qubits_col + self.image_color_base+index_bit]))
            for i in range((self.image_shape[0])):
                for j in range((self.image_shape[1])):
                    cur_position_binary = format(i, "b").zfill(self.num_qubits_row) + format(j, "b").zfill(
                        self.num_qubits_col)
                    for b in range(self.num_qubits - self.image_color_base- self.num_images_qubits):
                    ...
                    if self.image_color_base == 1:
                        circuit.append(cirq.ry(2 * params[self.image_color_base * (i * self.image_shape[1] + j)]).on(
                            bits[-((self.image_color_base+self.num_images_qubits))]).controlled_by(*bits[:-(self.image_color_base+self.num_images_qubits)]))
                        ...
                        circuit.append(
                            MCQI_Gate(bits, theta_r, theta_g, theta_b).on(*bits[-(self.image_color_base+self.num_images_qubits):-self.num_images_qubits]).controlled_by(
                                *bits[:-(self.image_color_base+self.num_images_qubits)]))
