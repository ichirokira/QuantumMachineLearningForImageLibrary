...
def filter_class(x, y, config, train=True):
    N, H,W,C = x.shape
    x_split = []
    y_split = []
...
def filter_nerq(x, y, config, num_samples=1000, train=True):
    N, H, W, C = x.shape

    keep0 = (y == config.CLASSES[0])

    keepx = keep0.reshape(keep0.shape[0])
    x0, y0 = x[keepx], y[keep0]
    keep1 = (y == config.CLASSES[1])
...
def train(config):
    ...
    if config.ENCODER == 'FRQI':
        ...
        qnn_layer = Multiview_FRQI(config, image_shape=(H, W, C))
