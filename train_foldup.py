...
def train(config):
    ...
    if config.ENCODER == 'FRQI':
        ...
        N, L, H, W, C = x_train.shape

        print("[INFO] Patchified Shape: ", x_train.shape)

        qnn_layer = FoldUp_FRQI(config, image_shape=(H, W, C))
        ...
        x_train = tf.image.extract_patches(x_train, sizes=[1, config.NUM_FOLD, config.NUM_FOLD, 1],
                                           strides=[1, config.NUM_FOLD, config.NUM_FOLD, 1], rates=[1, 1, 1, 1],
                                           padding='SAME')
        x_train = tf.transpose(x_train, perm=[0, 3, 1, 2])
        x_test = tf.image.extract_patches(x_test, sizes=[1, config.NUM_FOLD, config.NUM_FOLD, 1],
                                          strides=[1, config.NUM_FOLD, config.NUM_FOLD, 1], rates=[1, 1, 1, 1],
                                          padding='SAME')
        x_test = tf.transpose(x_test, perm=[0, 3, 1, 2])
        ...
        qnn_layer = FoldUp_NEQR(config, image_shape=(H, W, C), color_qubits=color_qubits)
